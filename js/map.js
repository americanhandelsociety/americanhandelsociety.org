$(function () {
  ...
  MapObj.prototype = {
    ...
    initiateMap: function () {
      ...
      var tiles = L.mapbox.styleLayer(
        "mapbox://styles/datamade/ckhdoudap025v19o2bfmiwcis",
        {
          accessToken:
            "pk.eyJ1IjoiZGF0YW1hZGUiLCJhIjoiaXhhVGNrayJ9.0yaccougI3vSAnrKaB00vA",
          attribution:
        ...
        this._geocoder.geocode(
          ...
          function (results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
              ...
              var latlng = L.latLng(
                $this.currentPinpoint[0],
                $this.currentPinpoint[1]
              );

              $.getJSON(
                "assets/americanhandelsociety_map.geojson",
                function (data) {
    ...
    setZoom: function (radius) {
      var zoom = "";
      if (radius >= 8050) zoom = 11;
      // 5 miles
      else if (radius >= 3220) zoom = 12;
      // 2 miles
      else if (radius >= 1610) zoom = 13;
      // 1 mile
      else if (radius >= 805) zoom = 14;
      // 1/2 mile
      else if (radius >= 400) zoom = 15;
      // 1/4 mile
      else zoom = 15;

      this.map.setView(
        new L.LatLng(this.currentPinpoint[0], this.currentPinpoint[1]),
        zoom
      );
    },

    addCircle: function (radius) {
      this.radiusCircle = L.circle(this.currentPinpoint, Number(radius), {
        opacity: 1,
        color: "#91a8ba",
        weight: 1,
        fillOpacity: 0.5,
    ...
    onEachFeature: function (feature, layer) {
      ...
      if (feature.properties) {
        ...
        layer.on("click", function (e) {
          ...
          $(
            ".short-description, #address-header, #long-description, #info-url"
          ).empty();

          // Add new content
          $(".short-description").append(data.short_description);
          $("#address-header").append(
          ...
          if (data.info_url) {
            infoUrl =
              "<a href='" +
              data.info_url +
              "' target='_blank'><strong><i class='fa fa-info-circle' aria-hidden='true'></i> Learn more</strong></a>";
        ...
        layer.on("mouseover", function (e) {
          ...
          var short_description =
            "<h4 class='infoBox-title'> " + data.short_description + "</h4>";
          var address =
          ...
          $("html")
            .find("div.infoBox")
            .html(short_description + address);
        });
        ...
        layer.on("mouseout", function () {
          ...
          $("html")
            .find("div.infoBox")
            .html("Click on a location to learn more");
        });
      }
    },
  }; // Close MapObj
